...
exp1=np.array(["Janela","Energia Consumida","Hora Inicial", "Hora Final","Flag Potência(0-nada associado,1-VB>5kWh,2-DMI>5kWh)","Consumo Voltbras"]).reshape(1,-1)
exp2=np.array(["Codigo","Energia Consumida","Hora Inicial", "Hora Final","Conector"]).reshape(1,-1)
exp3=np.array(["Codigo","Energia Consumida","Hora Inicial", "Hora Final","Conector","Motivo","Janela","Diferença(+ DMI, -VLBR"]).reshape(1,-1)
...
for i in range(1,vlbr.shape[0]):
    if int(vlbr[i,vlbr.shape[1]-3])==0:
        ...
        exp3=np.append(exp3,np.array([vlbr[i,5],vlbr[i,4],vlbr[i,8],vlbr[i,9],vlbr[i,6],"Sem Relação DMI","-","-"]).reshape(1,-1),axis=0)

for chave in dicrec:
    aux=int(dicrec[chave]["vlbr_eventos"])
    if aux>0:
        mataux=dicrec[chave]["vlbr"]
        for i in range(mataux.shape[0]):
            if float(mataux[i,4])==float(0):
                exp3=np.append(exp3,np.array([mataux[i,5],mataux[i,4],mataux[i,8],mataux[i,9],mataux[i,6],"Associado mas 0",chave,dicrec[chave]["dmi_energia"]-dicrec[chave]["vlbr_energia"]]).reshape(1,-1),axis=0)
...
with pd.ExcelWriter(file_path, engine='xlsxwriter') as writer:
    ...
    pd.DataFrame(exp3).to_excel(writer, sheet_name='Erros_Recargas_Globais', index=False, header=False)
